...
def build_explain_new_fact_prompt(
    *,
    ...
    # Silence rules 3 and 4 route here with no visitor utterance; "answer the user's
    # question" would then have nothing to answer. Note user_text is also empty in that
    # case, so select_new_facts falls back to its force_top1_below_low path -- the fact
    # choice is unaffected, only the framing sentence needs to change.
    silence_lead = f"{SILENCE_OPENER} {SILENCE_NO_PARAPHRASE}"
...
1) {silence_lead if is_silence else "First, directly answer the user's question in a natural way."}
...
ExplainNewFact policy for this turn:
1) {silence_lead if is_silence else "First, answer the user's question naturally."}
...
ExplainNewFact policy for this turn:
1) Sentence 1: {silence_lead if is_silence else "briefly answer the user's question."}
