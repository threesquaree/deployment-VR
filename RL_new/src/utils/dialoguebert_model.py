...
class DialogueBERTModel(nn.Module):
    ...
    def __init__(self, base_model_name: str = "bert-base-uncased", 
        ...
        # Initialize turn and role embeddings (small random, std=0.02).
        #
        # SERVING FIX: seed this init deterministically so every server
        # restart produces identical turn/role vectors. Without this, each
        # process draws fresh randoms, so the state features would differ
        # slightly between study participants across restarts. (The exact
        # training-time vectors are unrecoverable — they came from an
        # unseeded draw mid-startup — but at std=0.02 they are a ~2%
        # additive component; determinism across sessions is what matters.)
        # fork_rng keeps the global RNG state untouched.
