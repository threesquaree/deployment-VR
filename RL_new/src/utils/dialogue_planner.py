...
def build_prompt(option: str, subaction: str, ex_id: Optional[str],
                ...
                conversation_history_rows: Optional[List[dict]] = None,
                is_silence: bool = False) -> str:
    """Build structured prompt for LLM dialogue generation.

    is_silence marks a turn the visitor did not trigger (the silence timer). Policies
    whose first sentence answers the visitor have no referent on such a turn and fall
    back to paraphrasing the agent's own previous reply, so they are reworded instead.
    """
    
    # H6: Handle coarse option granularity by mapping coarse options to original option names
    # for prompt construction purposes. RecoverEngagement is Engage's OWN subaction
    # (deadend_fix_v1 action space) and must NOT be remapped — the guard also keeps
    # us off the option_configs import, which is not shipped in this deployment.
    ...
    if option == "Explain":
        ...
        if subaction == "AskOpinion":
            return build_ask_opinion_prompt(ex_id, context_section, facts_all, facts_used, current_completion, is_silence=is_silence)
        ...
        if subaction == "SummarizeAndSuggest":
            return build_summarize_and_suggest_prompt(ex_id, context_section, facts_all, facts_used, exhibit_names, knowledge_graph, target_exhibit, coverage_dict, is_silence=is_silence)
        ...
        if subaction == "RecoverEngagement":
            return build_recover_engagement_prompt(ex_id, context_section, facts_all, facts_used, current_completion, is_silence=is_silence)
...
def _build_enhanced_context_section(ex_id: Optional[str], last_utt: str, facts_all: List[str],
    ...
    # HANDOFF.md NLG checklist (b): the current exhibit's un-mentioned facts (with
    # their [FACT_ID] tags) must reach the prompt on every turn. Explain turns get
    # them through the fact selector (show_facts=True); the non-Explain actions
    # previously saw only the undifferentiated all-exhibit dump, which invited
    # repetition and untracked fact leakage.
    exhibit_facts_like = ""
    if not show_facts and ex_id and facts_all:
        facts_used = facts_used or []
        unmentioned = [f for f in facts_all if f not in facts_used]
        parts = []
        if unmentioned:
            parts.append(
                "Facts at this exhibit not yet shared (use only if the visitor's question "
                "needs one, and keep its [FACT_ID] tag): " + " | ".join(unmentioned)
            )
        if facts_used:
            parts.append(
                "Facts already shared at this exhibit (do not repeat as new): "
                + " | ".join(facts_used)
            )
        exhibit_facts_like = " ".join(parts)
    ...
    try:
        ...
        return render_base_prompt(
            ...
            user_input=last_utt,
            exhibit_facts_like=exhibit_facts_like,
        )
    except Exception:
...
def build_explain_new_fact_prompt(ex_id: Optional[str], context_section: str,
                                ...
                                user_text: str = "", is_silence: bool = False) -> str:
...
def build_recover_engagement_prompt(ex_id: Optional[str], context_section: str,
    ...
    try:
        from prompt.RecoverEngagement import build_recover_engagement_prompt as _build_prompt
        return _build_prompt(
            ex_id=ex_id,
            context_section=context_section,
            current_completion=current_completion,
...
1. The visitor's attention is fading. Do NOT introduce new facts.
2. Ask exactly one light, open opinion/perspective question about something already
   shown at this exhibit, including an exhibit or AOI keyword.
3. Text-only guide: never offer to show, play, demonstrate, or take the visitor anywhere.
4. Warm tone, two sentences maximum, no [FACT_ID], do not quote the visitor.
...
def build_ask_opinion_prompt(ex_id: Optional[str], context_section: str,
    ...
    try:
        ...
        # The fallback must carry the same silence wording; an import failure here
        # would otherwise silently restore the echo this fix exists to remove.
        fallback_lead = (
            "The visitor has gone quiet and asked nothing. Do not answer or invent a "
            "question, and do not restate or paraphrase your previous reply."
            if is_silence
            else "First respond naturally to the user's input."
        )
        return f"""[CONTEXT - DO NOT REPEAT]
Museum guide at: {ex_id} | Progress: {current_completion:.1%} covered
---
