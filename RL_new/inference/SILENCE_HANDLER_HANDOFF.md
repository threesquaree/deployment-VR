...
When a visitor is silent for the configured timeout, the system picks an agent action using a simple rule table (bypassing the RL model). This re-engages the visitor with a contextually appropriate response. After 2 consecutive silence-triggered actions without visitor input, it stops and waits.
...
```
THRESHOLD = configured_timeout_seconds
MAX_TRIGGERS = 2
...
Silence:  configured silence timeout → rule table selects action → build prompt → LLM generates utterance → agent speaks
```
...
|-----------|---------|-------|
| Threshold | Configured in Unity/runtime | Time before first trigger |
